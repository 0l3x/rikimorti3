...
public class EpisodeService {
    ...
    public static void listarTodosLosEpisodios() {
        try (Session session = HibernateUtils.getSession()) {
        	 List<Object[]> resultados = session.createNativeQuery(
        	            "SELECT id, name, air_date, episode FROM episode"
        	        ).list();
    ...
    public static void buscarEpisodiosPorTexto() {
        ...
        try (Session session = HibernateUtils.getSession()) {
            List<Object[]> resultados = session.createNativeQuery(
                    "SELECT id, name, air_date, episode FROM episode WHERE LOWER(name) LIKE LOWER(:text)")
        ...
        try (Session session = HibernateUtils.getSession()) {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (tx != null) tx.rollback();
                    System.out.println("Error de persistencia con character_in_episode: " + e.getMessage());
                } catch (Exception e) {
                    if (tx != null) tx.rollback();
                    System.out.println("Error con character_in_episode: " + e.getMessage());
                }
            }
